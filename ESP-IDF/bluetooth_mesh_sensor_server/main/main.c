...
// Define the GPIO for the Home Button
#define HOME_BUTTON_GPIO 3

#define TAG "BLE MESH OUTPUT"

// Add cooldown constant
#define BUTTON_COOLDOWN_MS 5000  // 5 seconds cooldown between presses to prevent spamming

#define CID_ESP 0x02E5

/* Sensor Property ID */
#define SENSOR_PROPERTY_ID_0 0x0056 /* Sensor Property */

static uint8_t sensor_data = 0; // Initially, the sensor value is 0

static uint8_t last_sensor_state = 0;  // Default state is 0

// Add cooldown variable
static int64_t last_press_time = 0;  // Last time the button was pressed
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags, uint32_t iv_index)
{
    ...
    /* Initialize sensor*/
    net_buf_simple_add_u8(&sensor_data_0, sensor_data);
...
static void example_ble_mesh_send_sensor_descriptor_status(esp_ble_mesh_sensor_server_cb_param_t *param)
{
    ...
    if (param->value.get.sensor_descriptor.op_en == false)
    {
        ...
        for (i = 0; i < ARRAY_SIZE(sensor_states); i++)
        {
            ...
            // Assuming descriptor.raw_value holds the temperature data in the correct byte format
            // Ensure that the data you send is correctly formatted
            uint16_t raw_value = sensor_states[i].sensor_data.raw_value->data[0] << 8 | sensor_states[i].sensor_data.raw_value->data[1];

            // You can just send the raw value if the client decodes it, no need to convert it on the server
            ESP_LOGI(TAG, "Raw temperature value: %d", raw_value);
...
// Timer callback to check for state changes
static void publish_sensor_data(void *arg)
{
    // Get current button state
    uint8_t current_state = (gpio_get_level(HOME_BUTTON_GPIO) == 0) ? 1 : 0;
    ...
    if (current_state != last_sensor_state) {
        // If button is being pressed (transitioning to 1)
        if (current_state == 1) {
            // Check if enough time has passed since last press
            if (current_time - last_press_time < BUTTON_COOLDOWN_MS) {
                // Ignore this press - still in cooldown period
                return;
            }
        ...
        sensor_data = current_state;  // Update sensor_data (global variable)
        
        // Reset the buffer and add the new sensor data
        net_buf_simple_reset(&sensor_data_0);
        net_buf_simple_add_u8(&sensor_data_0, sensor_data);

        // Create a context for sending the mesh message (broadcast in this example)
        esp_ble_mesh_msg_ctx_t ctx = {
            .addr = 0xFFFF,  // Broadcast address (change if needed)
            .app_idx = 0,
            .net_idx = 0,
            .recv_ttl = 0,
            .send_ttl = 0,
        ...
        // Send the updated sensor data via BLE Mesh
        esp_err_t err = esp_ble_mesh_server_model_send_msg(sensor_server.model, &ctx,
                                                           ESP_BLE_MESH_MODEL_OP_SENSOR_STATUS,
                                                           sensor_data_0.len, sensor_data_0.data);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to send sensor data");
        } else {
            ESP_LOGI(TAG, "Published sensor data: %d", sensor_data);
        }
...
static esp_err_t create_periodic_timer(void)
{
    esp_timer_create_args_t timer_args = {
        .callback = publish_sensor_data,
        .name = "sensor_publish_timer"};
    ...
    // Start the timer with a period of 1 second (1000000 microseconds) / 1 MS (100000 microseconds)
    err = esp_timer_start_periodic(periodic_timer, 100000); // 1 MS interval for polling
    if (err != ESP_OK)
    {
        ESP_LOGE("Timer", "Failed to start periodic timer");
        return err;
    }
...
{
    esp_rom_gpio_pad_select_gpio(HOME_BUTTON_GPIO);
    gpio_set_direction(HOME_BUTTON_GPIO, GPIO_MODE_INPUT);
    gpio_set_pull_mode(HOME_BUTTON_GPIO, GPIO_PULLUP_ONLY); // Pull-up resistor
}
...
void app_main(void)
{
    ...
    init_gpio(); // Initialize the Home button GPIO
    ...
    // Start the periodic sensor publishing
    err = create_periodic_timer();
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to create periodic timer");
    }
