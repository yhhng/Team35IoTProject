...
#define TAG "BLE_MESH_SENSOR_CLIENT"
...
// Define GPIO pin for the Home button
...
// Define the PSK Key
static const uint8_t s_key[] = PSK_KEY;

// MQTT client initialization (using async MQTT for non-blocking)
...
// MQTT Portion (Wifi and MQTT) Initialization
...
// Forward sensor data to MQTT broker
void forward_sensor_data_with_topic(uint8_t *sensor_data, uint16_t length, const char *topic)
{
    if (length == 1)
    {
        // For one byte of data, it should be 0x00 or 0x01 only
        uint8_t val = sensor_data[0];
        char payload[2]; // Enough for '0' or '1' plus null terminator
...
static esp_err_t mqtt_event_handler_cb(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        esp_mqtt_client_publish(client, "connection_status", "connected", 0, 1, 0);
        // Initialise both sensor data to 0 on first connection
        esp_mqtt_client_publish(client, "home/bedroom/bed/piezzo_sensor_data", "0", 0, 1, 0);
        esp_mqtt_client_publish(client, "home/livingroom/sofa/piezzo_sensor_data", "0", 0, 1, 0);
        break;
...
void example_ble_mesh_send_sensor_message(uint32_t opcode)
{
    ...
    case ESP_BLE_MESH_MODEL_OP_SENSOR_SERIES_GET:
        ...
        break;
    case ESP_BLE_MESH_MODEL_OP_SENSOR_GET: // To retrieve sensor data
                                              ...
                                              esp_ble_mesh_sensor_client_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_SENSOR_CLIENT_PUBLISH_EVT:
        ...
        ESP_LOG_BUFFER_HEX("Published Sensor Data", param->status_cb.sensor_status.marshalled_sensor_data->data,
                           param->status_cb.sensor_status.marshalled_sensor_data->len);

        ESP_LOGI(TAG, "Sensor data length: %d", param->status_cb.sensor_status.marshalled_sensor_data->len);

        // Select topic based on node address
        if (node_addr == 0x0005)
        {
            topic = "home/livingroom/sofa/piezzo_sensor_data";
        }
        else if (node_addr == 0x0006)
        {
            topic = "home/bedroom/bed/piezzo_sensor_data";
        }
        else
        {
            // For additional node; to test out if the data is being published to the MQTT broker
            static char fallback[64];
            sprintf(fallback, "home/unknown/0x%04X/piezzo_sensor_data", node_addr);
            topic = fallback;
        }

        // Forward the sensor data to the chosen MQTT topic after conversion.
        forward_sensor_data_with_topic(
