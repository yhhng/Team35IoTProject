...
void setup() {
  ...
  Serial.println("Kitchen's Toilet Rug Node Ready");
}
  ...
  if (digitalRead(HOME_BUTTON_PIN) == 0) {
    ...
    if (currentTime - lastButtonPressTime > buttonDebounceDelay) {
      ...
      if (currentPinState != previousPinState) {
        ...
        client.publish(topicKitchenToiletFallDectector, encryptedMsg.c_str());
...
void reConnect() {
  while (!client.connected()) {
    ...
    String clientId = "KitchenToiletFallDetectorSensor-" + String(random(0xffff), HEX);
