...
void setup() {
  ...
  M5.Lcd.println("Node A (Living Room) Ready");
}
...
void loop() {
  ...
  if (sensorValue != lastSensorValue) {
    ...
    client.publish(topicLivingroom, encryptedMsg.c_str());

    // LCD update
    M5.Lcd.fillScreen(BLACK);
    M5.Lcd.setCursor(0, 0);
    M5.Lcd.print("Living Room's Door IR Sensor: ");
...
void reConnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_username, mqtt_password)) {
      M5.Lcd.println("\nConnected");
      client.subscribe("home/livingroom/door/cmd");
    } else {
